...
def user_profile(request):
    ...
    # 获取待审批申请（仅管理员和超级管理员可见）
    # 超级管理员：能看到所有待审批申请
    # 管理员（Moderators）：只能看到编辑组的待审批申请
    ...
    if request.user.is_superuser:
        ...
        pending_applications = GroupApplication.objects.filter(status='pending', requested_group='Editors').order_by('-created_at')
