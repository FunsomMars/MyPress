...
class BlogIndexPage(Page):
    ...
    def get_context(self, request, *args, **kwargs):
        ...
        # 添加年份相关变量
        if archive_list:
            years = [year for year, _ in archive_list]
            context['min_year'] = min(years)
            context['max_year'] = max(years)
            # 如果没有筛选，使用最新的年份
            if not filter_year:
                context['current_year'] = years[0]  # 最新年份
            else:
                context['current_year'] = int(filter_year)
